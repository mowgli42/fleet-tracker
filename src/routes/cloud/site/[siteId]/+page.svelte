<script lang="ts">
  ...
  const jobs = $derived(site ? jobsForSite(site.id, fleet) : []);
  const readyN = $derived(site ? readyForAssignmentCount(site.id, fleet) : 0);
</script>
...
<div class="space-y-6">
  ...
  {:else}
    ...
    <div class="grid gap-3 sm:grid-cols-3">
      ...
      <div class="cloud-card p-4">
        <p class="m-0 text-[10px] font-medium uppercase tracking-wide" style="color: var(--cloud-muted);">Open jobs</p>
    ...
    <div class="cloud-card p-4">
      <h2 class="m-0 font-display text-base font-semibold" style="color: var(--cloud-text);">Maintenance (site)</h2>
      {#if jobs.length === 0}
        <p class="mt-2 text-sm" style="color: var(--cloud-muted);">No jobs for vehicles at this site.</p>
      {:else}
        <ul class="mt-3 space-y-2">
          {#each jobs.filter((j) => j.status !== 'completed') as j}
            <li class="rounded-md border px-3 py-2 text-sm" style="border-color: var(--cloud-border);">
              <div class="flex flex-wrap items-center justify-between gap-2">
                ...
                <span class="text-[11px] uppercase" style="color: var(--cloud-muted);">{j.status}</span>
              </div>
              <p class="m-0 mt-1 text-xs" style="color: var(--cloud-muted);">{j.priority} · {j.component ?? '—'}</p>
            </li>
          {/each}
        </ul>
      {/if}
